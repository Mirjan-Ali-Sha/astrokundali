...
# Enhanced Ashtakoota (Guna Milan) marriage matching with dynamic interpretation, faults, and remedies.
# """
# from .astro_data import AstroData
# from .dispositions import get_dispositions

# # Mappings for koota types and significances
# VARNA_MAP = {1:3,2:2,3:1,4:3,5:4,6:2,7:1,8:3,9:4,10:2,11:1,12:4}
# VARNA_NAME = {1:'Brahmin',2:'Kshatriya',3:'Vaishya',4:'Shudra'}
# VASHA_MAP = {1:'Chatushpada',2:'Manav',3:'Jalachara',4:'Jalachara',
#              5:'Chatushpada',6:'Manav',7:'Manav',8:'Vanachara',
#              9:'Manav',10:'Chatushpada',11:'Chatushpada',12:'Jalachara'}
# GANA_GROUPS = {**{n:'Deva' for n in [2,6,10,11,15,19,23,27]},
#                **{n:'Manushya' for n in [1,5,7,12,13,17,20,22,24]},
#                **{n:'Rakshasa' for n in set(range(1,28)) - {2,6,10,11,15,19,23,27} - {1,5,7,12,13,17,20,22,24}}}
# YONI_MAP = {1:'Horse',2:'Elephant',3:'Sheep',4:'Snake',5:'Sheep',6:'Dog',
#             7:'Cat',8:'Rat',9:'Cow',10:'Rat',11:'Buffalo',12:'Tiger',
#             13:'Lion',14:'Mongoose',15:'Deer',16:'Monkey',17:'Tiger',
#             18:'Dog',19:'Elephant',20:'Horse',21:'Dog',22:'Horse',
#             23:'Sheep',24:'Rat',25:'Deer',26:'Rat',27:'Elephant'}
...
# # Koota weights and descriptions
# KOOTA_INFO = {
#     'Varna':      {'max':1,  'desc':'Spiritual Development'},
#     'Vashya':     {'max':2,  'desc':'Mutual Control Compatibility'},
#     'Tara':       {'max':3,  'desc':'Health and Longevity'},
...
# REMINDERS = {
#     'Varna': ['Perform Navagraha Shanti Puja', 'Recite Gayatri Mantra'],
#     'Vashya': ['Donate sweets/clothes', 'Wear Cat’s Eye gemstone'],
#     'Tara': ['Tara Dosha Nivaran Havan', 'Visit Tara Devi temple'],
#     'Yoni': ['Chant Maha Mrityunjaya Mantra', 'Perform Graha Shanti Pooja'],
#     'Gana': ['Perform Gan Dosh Nivaran Puja', 'Chant Jupiter Mantra'],
#     'Bhakoot': ['Bhakoot Dosha Puja', 'Charity of white foods'],
#     'Graha Maitri': ['Graha Maitri Shanti Havan', 'Wear Pearl & Yellow Sapphire'],
#     'Nadi': ['Nadi Dosha Havan', 'Pilgrimage to Saptashrungi'],
#     'Manglik': ['Rice puja at Mangalnath', 'Plant neem tree', 'Donate red items on Tuesday']
# }
...
# # Scoring functions
# def varna_koota(m1,m2): return 1 if VARNA_MAP[m1['sign_number']]>=VARNA_MAP[m2['sign_number']] else 0

# def vashya_koota(m1,m2): return 2 if VASHA_MAP[m1['sign_number']]==VASHA_MAP[m2['sign_number']] else 0

# def tara_koota(m1,m2):
#     n1,n2=m1['nakshatra'],m2['nakshatra']
#     k1=(n2-n1)%9 or 9; k2=(n1-n2)%9 or 9
#     def sc(k): return 3 if k in (1,9) else 1.5 if k in (3,5) else 0
#     return sc(k1)+sc(k2)

# def yoni_koota(m1,m2): return 4 if YONI_MAP[m1['nakshatra']]==YONI_MAP[m2['nakshatra']] else 0

# def gana_koota(m1,m2):
#     g1,g2=GANA_GROUPS[m1['nakshatra']],GANA_GROUPS[m2['nakshatra']]
#     if g1==g2: return 6
#     if set([g1,g2])=={'Deva','Manushya'}: return 5
#     if set([g1,g2])=={'Manushya','Rakshasa'}: return 1
#     return 0
...
# def bhakoot_koota(m1,m2):
#     d=abs(m1['sign_number']-m2['sign_number']); return 7 if d in {1,2,4,6,7,8,10,11} else 0

# def nadi_koota(m1,m2): return 8 if NADI_MAP[m1['nakshatra']]!=NADI_MAP[m2['nakshatra']] else 0

# def manglik_dosha(data):
#     raw=data.get_rashi_data(); asc=raw['ascendant']['lon']
#     mars=raw['mars']['lon']%360
#     house=int((mars-asc)%360//30)+1
#     return house in (1,2,4,7,8,12)
...
# def match_kundli(a:AstroData,b:AstroData)->dict:
#     """Return detailed compatibility table, dynamic interpretation, faults, and remedies."""
#     d1,d2=get_dispositions(a),get_dispositions(b)
#     m1,m2=d1['moon'],d2['moon']
#     # compute each koota
#     scores={k:globals()[k.lower().replace(' ','_') + '_koota'](m1,m2) for k in KOOTA_INFO}
#     total=sum(scores.values()); max_total=sum(v['max'] for v in KOOTA_INFO.values())
#     # faults
#     faults=[k for k,v in scores.items() if v==0]
#     # manglik
#     mg_a,mg_b=manglik_dosha(a),manglik_dosha(b)
#     if mg_a!=mg_b: faults.append('Manglik')
#     # build table
#     table=[]
#     for k,pts in scores.items():
#         info=KOOTA_INFO[k]
#         table.append({
#             'Particular':f"{k} Koota",
#             'Max':info['max'],'Obtained':pts,
#             'Significance':info['desc']
#         })
#     table.append({
#         'Particular':'Total','Max':max_total,'Obtained':total,
#         'Significance':f"Overall Compatibility: {total/max_total*100:.1f}%"
#     })
...
#     # Manglik text
#     if 'Manglik' in faults:
#         s=f"Boy is {'Manglik' if mg_a else 'not Manglik'}, while Girl is {'Manglik' if mg_b else 'not Manglik'}."
#         if mg_a:
#             # check cancellation by aspect: assume Jupiter aspect function
#             s+= " However, if Mars is aspected by Jupiter in boy's chart, the Manglik Fault is canceled." 
#         interp.append(s)
#     # other faults
#     for f in faults:
#         if f=='Manglik': continue
#         desc=KOOTA_INFO[f]['desc']
#         rem=', '.join(REMINDERS.get(f,[]))
#         interp.append(f"{f} Koota scored zero ({desc}); remedies: {rem}.")
#     if not faults:
#         interp.append("All kootas are satisfied, indicating strong compatibility and a harmonious marriage.")
#     return {'table':table,'faults':faults,'remedies':{f:REMINDERS.get(f,[]) for f in faults},'interpretation':' '.join(interp)}
